...
type CachedRoute = Omit<FlowParticlePlan, 'id' | 'systemId' | 'selected' | 'directional'> & { sourceOrderPreserved: boolean };
...
function plansForFeature(feature: Feature<Geometry, GeoJsonProperties>, selectedPoint: Coordinate | null, bounds?: FlowBounds): FlowParticlePlan[] {
  ...
  return routes.map((route, index) => ({ ...route, id: `${systemId}-route-${index}`, systemId, selected, directional: directionMetadataVerified && route.sourceOrderPreserved }));
}
...
function particleBudget(plans: FlowParticlePlan[], zoom: number): number {
  ...
  const spacingKm = selected
    ...
    : (zoom >= 9 ? 18 : zoom >= 7 ? 34 : 75);
  const cap = selected
    ? (zoom >= 9 ? 160 : zoom >= 7 ? 110 : 64)
    : (zoom >= 9 ? 140 : zoom >= 7 ? 90 : 48);
...
export function createFlowParticleCollection(
  ...
  const seedEveryRoute = plans.length <= budget;
  if (seedEveryRoute) {
    for (const plan of plans) allocations.set(plan, [plan.lengthKm / 2]);
  }
  const distributedBudget = budget - (seedEveryRoute ? plans.length : 0);
  let routeStart = 0;
  let planIndex = 0;
