...
export const Sidebar: React.FC = () => {
  ...
  return <aside className={`hydro-sidebar ${isLight ? 'light-scrollbar' : ''}`}>
    <div className="hydro-sidebar-header">
      <div className="hydro-sidebar-titlebar"><div><div className="hydro-sidebar-title"><Activity className="h-4 w-4 text-cyan-400" />HESLER</div><div className="hydro-sidebar-meta">20 MW+ · TATUS · EPSG:4326 · {dataStatus === 'loading' ? 'yükleniyor' : 'kanonik veri'}</div></div><button onClick={() => useHydrologyStore.getState().toggleSidebar()} className="hydro-sidebar-close" aria-label="Paneli kapat"><X className="h-4 w-4" /></button></div>
      <div className="hydro-search-wrap"><Search className="hydro-search-icon" /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="HES, akarsu veya havza ara..." className="hydro-search" /></div>
    </div>
    ...
    <div className="hydro-sidebar-footer">20 MW+ HES envanteri · seçim haritada uygun ölçeğe yaklaşır · doluluk: gerçek kaynak resolver&apos;ı</div>
  </aside>;
};
